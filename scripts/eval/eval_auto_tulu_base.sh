...
python3 -m eval.gsm.run_eval \
    --data_dir data/eval/gsm/ \
    --save_dir results/gsm/${MODEL_NAME}-cot-tulu-8shot \
    --model $CHECKPOINT_PATH \
    --tokenizer $CHECKPOINT_PATH \
    --n_shot 8 \
    --use_vllm \
...
python3 -m eval.mgsm.run_eval \
    ...
    --save_dir results/mgsm/${MODEL_NAME}-cot-en-tulu-8shot \
    --model $CHECKPOINT_PATH \
    --tokenizer $CHECKPOINT_PATH \
    --n_shot 8 \
    --mode cot-en \
...
python3 -m eval.mgsm.run_eval \
    --data_dir data/eval/mgsm/ \
    --save_dir results/mgsm/${MODEL_NAME}-trans-cot-tulu-8shot \
    --model $CHECKPOINT_PATH \
    --tokenizer $CHECKPOINT_PATH \
    --n_shot 8 \
    --mode trans-cot \
...
python3 -m eval.codex_humaneval.run_eval \
    ...
    --temperature 0.8 \
    --save_dir results/codex_humaneval/{$MODEL_NAME}_tulu-temp_0_8 \
    --model $CHECKPOINT_PATH \
    --tokenizer $CHECKPOINT_PATH \
    --use_vllm \
    --use_chat_format \
...
python3 -m eval.mmlu.run_eval \
    ...
    --save_dir results/mmlu/${MODEL_NAME}-tulu-5shot \
